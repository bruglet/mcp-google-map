# Historical upstream architecture reference

> This file is retained to ease upstream synchronization. For the current
> fork, `docs/ARCHITECTURE.md`, `docs/DEPLOYMENT.md`, and
> `UPSTREAM_DEVIATIONS.md` are authoritative. Sections describing weather,
> disabled tools, header-based key precedence, or old tool-count checklists
> are historical unless they agree with those fork documents.
...
## API Key Management (fork policy)

This inherited reference is retained for upstream comparison only. It is not
the active security model. The fork reads `GOOGLE_MAPS_API_KEY` and the
optional `GOOGLE_MAPS_GROUNDING_API_KEY` from the protected server process
environment. It rejects request headers, bearer tokens, CLI key arguments,
cookies, URLs, logs, health output, and ledger state as credential sources.

The deployed HTTP origin separately validates the Cloudflare Access
`Cf-Access-Jwt-Assertion` header. See `docs/ARCHITECTURE.md` and
`docs/DEPLOYMENT.md` for the authoritative flow.
