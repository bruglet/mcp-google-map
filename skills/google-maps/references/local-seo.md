# Historical upstream research — Local SEO

> This document is retained for attribution and upstream comparison. It is
> not an active v1 API guide: the current fork does not expose local-rank,
> static-map, or photo/maxPhotos tools, and it does not promise Google Search
> ranking analysis. Use `skills/google-maps/references/tools-api.md` and
> `docs/COST_POLICY.md` for the current tool contract.
